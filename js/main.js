...
const MEMBER = new THREE.Vector3(288, 0.5, 10);
...
const curve = new THREE.CatmullRomCurve3(
  [
    ...
    new THREE.Vector3(271, 0.5, 3.4),
    new THREE.Vector3(280, 0.5, 7),
...
function bank() {
  ...
  for (const sgn of [-1, 1]) {
    for (let i = 0; i < 6; i++) {
      ...
      if (i < 5) {
        g.add(box(1.15, 3.2, 0.12, MAT.dark, lx + 1.08, 4.1, sgn * 4.56));
      }
    }
  }
...
const switchFlash = (() => {
  ...
  m.position.set(HQ_POS.x - 4.7, 1.6, HQ_POS.z);
...
function tick() {
  ...
  beamLight.intensity = 60 * charge * (0.35 + 0.65 * vis);

  /* coral -> cyan hand-over flash at the HQ entrance */
  const flashAmt = Math.exp(-Math.pow((t - T_SWITCH) * 80, 2));
